...
void main(void) {
	...
	vec3 N = (normalize(translate * vec4(v_Normal,1.0))).xyz;

	vec3 L = normalize(-translate * vec4(camera,0)).xyz;
	vec3 E = normalize(-V);
	...
	vec3 specular	= clamp(vec3(0.0, 0.0, 0.20) * pow(max(dot(R,E),0.0), 0.1 ), 0.0, 1.0);
