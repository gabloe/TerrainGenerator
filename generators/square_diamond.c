...
void normal( float* vec1 , float* vec2 , float* vec3 , float* norm ) {
	...
	float div = (float)sqrt( norm[0] * norm[0] + norm[1]*norm[1] + norm[2]*norm[2] );
...
void main(int argc , char** args) {
	...
	int MAX_INDEX, MESH_SIZE, MAX_SIZE;
	...
	while( size > 1) {
		...
		for( x = 0 ; x < MAX_INDEX ; x += size ) {
			for( y = 0 ; y < MAX_INDEX ; y += size ) {
				...
				set( data , x + offset , y + offset , (float)(comp * 0.25 ));
			}
		}
		// Diamond
	...
	_write( data , MESH_SIZE , _min , _max );

#if VALIDATE == 1
	_validate( data , MESH_SIZE , _min , _max );
#endif
	free( data );
	return;
}
...
void _validate(float *data , short MESH_SIZE , float _min , float _max ) {
	...
	for (i = 0; i < MESH_SIZE; i++) {
		for (j = 0; j < MESH_SIZE; j++) {
			float t, x = (get(data, i, j) - _min) / (_max - _min);
